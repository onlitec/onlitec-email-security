...
exports.list = async (req, res) => {
    try {
        ...
        let query = `SELECT id, tenant_id, type, value, comment, created_at FROM blacklist WHERE 1=1`;
        const params = [];
        let paramIndex = 1;
...
exports.create = async (req, res) => {
    try {
        ...
        const existing = await pool.query('SELECT id FROM blacklist WHERE tenant_id = $1 AND type = $2 AND value = $3', [tenant_id, type, value]);
        if (existing.rows.length > 0) {
            return res.status(409).json({ success: false, error: { code: 'DUPLICATE', message: 'Entry already exists' } });
        }

        const result = await pool.query(`
            INSERT INTO blacklist (tenant_id, type, value, comment) VALUES ($1, $2, $3, $4) RETURNING *
