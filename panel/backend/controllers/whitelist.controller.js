...
exports.list = async (req, res) => {
    try {
        ...
        let query = `SELECT id, type, value, reason, created_by, created_at FROM whitelist WHERE 1=1`;
        const params = [];
        let paramIndex = 1;
...
exports.create = async (req, res) => {
    try {
        ...
        const existing = await pool.query('SELECT id FROM whitelist WHERE type = $1 AND value = $2', [type, value]);
        if (existing.rows.length > 0) {
            return res.status(409).json({ success: false, error: { code: 'DUPLICATE', message: 'Entry already exists' } });
        }

        const result = await pool.query(`
            INSERT INTO whitelist (type, value, reason, created_by) VALUES ($1, $2, $3, $4) RETURNING *
