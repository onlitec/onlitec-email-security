        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-gray-600">Manage email forwards and catch-all addresses</p>
                </div>
                            ...
                            <tr key={alias.id}>
                                ...
                                <td className="px-4 py-4">
                                    <span className={`px-2 py-1 text-xs rounded-full ${alias.is_catch_all ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800'}`}>
                                        {alias.is_catch_all ? 'Catch-All' : 'Forward'}
                                    </span>
                                </td>
                                <td className="px-4 py-4">
                                ...
                                <td className="px-4 py-4 text-right">
                                    <button onClick={() => handleDelete(alias.id)} className="text-red-600 hover:text-red-900">Delete</button>
                                </td>
                            </tr>
                        ))}
                        {aliases.length === 0 && (
                            <tr><td colSpan="6" className="px-4 py-4 text-center text-gray-500">No aliases found</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 w-full max-w-md">
                        <h3 className="text-lg font-medium mb-4">Add Email Alias</h3>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Email (alias address)</label>
                                <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                                    placeholder="alias@domain.com" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Destination (forward to)</label>
                            ...
                            <div className="flex items-center">
                                ...
                                <label className="ml-2 text-sm text-gray-700">Catch-all (receive all emails to this domain)</label>
                            </div>
