...
export default function Quarantine() {
    ...
    const handleRelease = async (id) => {
        if (!confirm('Release this email?')) return
        try {
            await api.post(`/quarantine/${id}/release`)
            fetchEmails()
        } catch (err) {
            alert(err.response?.data?.error?.message || 'Failed to release')
        }
    }
    ...
    const handleBulkRelease = async () => {
        ...
        if (!confirm(`Release ${selected.length} emails?`)) return
        try {
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-gray-600">Manage quarantined emails</p>
                </div>
                {selected.length > 0 && (
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {emails.map((email) => (
                            <tr key={email.id} className={selected.includes(email.id) ? 'bg-blue-50' : ''}>
                                ...
                                <td className="px-4 py-3 text-sm text-gray-500">{email.score?.toFixed(1) || '-'}</td>
                                <td className="px-4 py-3 text-sm text-gray-500">{formatDate(email.created_at)}</td>
                                <td className="px-4 py-3 text-right text-sm space-x-2">
                                    {email.status === 'quarantined' && (
                                        <button onClick={() => handleRelease(email.id)} className="text-green-600 hover:text-green-900">Release</button>
                                    )}
                                    <button onClick={() => handleDelete(email.id)} className="text-red-600 hover:text-red-900">Delete</button>
                                </td>
                            </tr>
                        ))}
                        {emails.length === 0 && (
                            <tr><td colSpan="8" className="px-4 py-8 text-center text-gray-500">No quarantined emails</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
